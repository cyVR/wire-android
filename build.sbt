...
lazy val processor = project.in(file("annotations-processor"))
  .settings(
    exportJars := true,
    libraryDependencies ++= Seq(autoService, commonsLang3, javaPoet, javaWriter),
...
lazy val wireCore = project.in(file("wire-core"))
  ...
  .dependsOn(annotations, processor % "provided")
...
lazy val root = Project("zclient", file(".")).aggregate(annotations, processor, wireCore, wireUi, app)
